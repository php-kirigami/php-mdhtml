 ...
 *
 * `emoji_builtin` is MINIT-initialized and MSHUTDOWN-destroyed: populated
 * once, never touched again, a straight C-array-to-HashTable copy of
 * MD::$emojiMap (kirigami/php-prepros), persistent memory throughout.
 *
 * `emoji_custom` / `plugins` (MD::registerEmoji() / MD::registerPlugin())
 * are request-scoped: RINIT-initialized, RSHUTDOWN-destroyed, request
 * (emalloc) memory. They hold request data -- the callers' key strings,
 * emoji strings and plugin Closures -- which the engine frees when the
 * request ends, so they must not outlive it. Like MD::'s userland statics,
 ...
 * v0.1.4 (point 16). Custom entries are looked up first, exactly like
 * MD::emojiFor().
 * ========================================================================
 */
 ...
 * looked up and invoked later, after this function's own call frame is
 * gone -- but only within the same request: the table is request-scoped,
 * see the MODULE GLOBALS comment above. */
...
/* Request-scoped registrations -- see the MODULE GLOBALS comment above.
 * Non-persistent tables: they hold request memory (keys, strings, Closures)
 * and are emptied while that memory is still valid. */
